(function (root, factory) {
  ...
  function reportHeaders() {
    return [
      "PODE ALOCAR?", "SITUAÇÃO", "DOCUMENTO", "ABA LD", "LINHA LD", "VERSÃO DA LD ENVIADA",
  ...
  function reportRow(result) {
    ...
    return [
      reportDecision(result), reportSituation(result), result && result.document || "", result && result.sheet || record.sheet || "", Number(record.row) || "",
  ...
  async function buildAnalysisReport(results, meta, ExcelJS) {
    ...
    const info = [
      ["LD analisada", settings.ldName || ""], ["Gerado em", settings.generatedAt || new Date().toLocaleString("pt-BR")],
    ...
    [24, 34, 46, 12, 11, 21, 24, 34, 58, 52, 18, 28, 78, 22, 58, 32, 23, 28, 68, 22, 35, 52, 72]
      .forEach((width, index) => { sheet.getColumn(index + 1).width = width; });
    sheet.autoFilter = { from: "A1", to: `W${Math.max(1, sheet.rowCount)}` };
    sheet.pageSetup = { orientation: "landscape", fitToPage: true, fitToWidth: 1, fitToHeight: 0, printTitlesRow: "1:1", margins: { left: .2, right: .2, top: .45, bottom: .45, header: .2, footer: .2 } };
  ...
  async function verify(buffer, expectedResults, allocationCode, ExcelJS) {
    ...
    const allocationSheet = allocationBook.getWorksheet("GERAL");
    if (!allocationSheet) throw new Error("A planilha GERAL não foi criada.");
