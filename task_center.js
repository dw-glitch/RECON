(function (root) {
  ...
  function inject() {
    ...
    document.body.insertAdjacentHTML("beforeend", '<div id="quality-task-overlay" class="task-center-overlay" hidden></div><aside id="quality-task-drawer" class="task-center-drawer" aria-hidden="true"><header><div><h2>Central de tarefas</h2><span id="quality-task-summary">Nenhuma tarefa</span></div><button id="quality-task-close" type="button" aria-label="Fechar">×</button></header><div id="quality-task-list" class="task-center-list"></div><footer><button id="quality-task-clear" type="button">Limpar concluídas</button></footer></aside>');
    const open = (visible) => { document.getElementById("quality-task-overlay").hidden = !visible; document.getElementById("quality-task-drawer").classList.toggle("open", visible); document.getElementById("quality-task-drawer").setAttribute("aria-hidden", String(!visible)); };
