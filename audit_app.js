/* RECON 1.26.10 — integração complementar do SCON ESCOPO. */
(function () {
  "use strict";
  ...
  function updateTitleReferenceStatus() {
    ...
    const sconEscopoText = sconEscopoCount ? `${sconEscopoCount.toLocaleString("pt-BR")} TAGs de referência no SCON ESCOPO` : "SCON ESCOPO indisponível";
    const extraText = extraCount ? ` · ${extraCount.toLocaleString("pt-BR")} referências adicionais` : "";
    els.titleBaseStatus.textContent = `${baseText} · ${sconText} · ${sconEscopoText}${extraText}`;
  ...
  function titleSourceCategory(row) {
    ...
    if (row.sconEscopoMatch === "SIM" && /^SCON ESCOPO/i.test(row.descriptionSource || "")) {
      return /DENTRO DO GRUPO 7/i.test(row.sconEscopoMatchMode || "") ? "scon_escopo_component" : "scon_escopo_exact";
    }
  ...
  function visibleRows(kind) {
    ...
    return rows.filter((row) => {
      ...
      if (search && !Q.norm(`${row.document} ${row.sheet} ${row.allocation || ""} ${row.controlledSourceKind || ""} ${row.controlledSourceFile || ""} ${row.title || ""} ${row.current} ${row.proposed} ${row.tag || ""} ${row.description || ""} ${row.descriptionSource || ""} ${row.sconTitleComplement || ""} ${row.sconFullDescription || ""} ${row.sconEscopoTitle || ""} ${(row.sconEscopoCandidateTitles || []).join(" ")} ${row.reason}`).includes(search)) return false;
      return true;
    });
  }
  ...
  function titleEvidenceHtml(row) {
    ...
    const sourceLabel = {
      ...
      scon_escopo_exact: "SCON ESCOPO · TAG exata",
    ...
    const sourceRows = (row.sconSourceRows && row.sconSourceRows.length ? row.sconSourceRows : row.sconEscopoSourceRows || []).join(", ");
    ...
    const sconEscopoCandidateText = sconEscopoCandidates.length
      ...
      : "";
    const sconEscopoConflictText = /DISCIPLINA/i.test(row.sconEscopoMatchMode || "")
  ...
  async function analyzeTitles() {
    ...
    try {
      setProgress("title", 32, "Carregando evidências SCON TAG SGP e SCON ESCOPO…");
  ...
  function titleReferenceSummary(row) {
    ...
    const references = [
      ...
      row.sconEscopoTitle ? `SCON ESCOPO: ${row.sconEscopoTitle}` : "",
    ].filter(Boolean);
  ...
  async function buildAuditWorkbook(kind) {
    ...
    if (kind === "title") {
      ...
      criteriaRows.push(["Ordem da busca", "1. Código documental SGP exato; 2. código sem diferenças de pontuação; 3. TAG completa do Grupo 7; 4. TAG no campo CÓDIGO SGP; 5. TAG principal contida em identificador maior. Nas buscas 3 a 5, a disciplina e a unicidade da descrição são obrigatórias"]);
      criteriaRows.push(["Regra da SCON ESCOPO", "Disciplina e área servem somente para localizar a referência. O título combina as atividades encontradas, sem esses campos, e mantém a TAG no final"]);
  ...
  async function loadBundledReferences() {
    const jobs = [
      ...
      Promise.resolve().then(() => {
        ...
        if (parsed.entries.length !== 7798 || parsed.uniqueTagCount !== 3027) {
          throw new Error(`O SCON ESCOPO contém ${parsed.entries.length} linhas e ${parsed.uniqueTagCount} TAGs; eram esperadas 7.798 linhas e 3.027 TAGs.`);
        }
